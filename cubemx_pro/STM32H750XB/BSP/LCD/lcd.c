...
//SDRAM_HandleTypeDef SDRAM_Handler;    //SRAM句柄(用于控制LCD)
//LCD的画笔颜色和背景色	   
...
//hsram:SRAM句柄
void HAL_SRAM_MspInit(SDRAM_HandleTypeDef *hsram)
{
...
//        HAL_GPIO_Init(GPIOB,&GPIO_Initure); 
        ...
        GPIO_Initure.Pin=GPIO_PIN_4;           //PB5,背光控制
        GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;  //推挽输出
        GPIO_Initure.Pull=GPIO_PULLUP;          //上拉
        GPIO_Initure.Speed=GPIO_SPEED_FREQ_VERY_HIGH;    //高速
        HAL_GPIO_Init(GPIOD,&GPIO_Initure); 
...
//        hsdram1.Instance=FMC_NORSRAM_DEVICE; //BANK1
//        hsdram1.Extended=FMC_NORSRAM_EXTENDED_DEVICE;       
//        
//        hsdram1.Init.SDBank=FMC_NORSRAM_BANK1;     //使用NE1
//        hsdram1.Init.DataAddressMux=FMC_DATA_ADDRESS_MUX_DISABLE;  //不复用数据线
//        hsdram1.Init.MemoryType=FMC_MEMORY_TYPE_SRAM;              //SRAM
//        hsdram1.Init.MemoryDataWidth=FMC_NORSRAM_MEM_BUS_WIDTH_16; //16位数据宽度
//        hsdram1.Init.BurstAccessMode=FMC_BURST_ACCESS_MODE_DISABLE; //是否使能突发访问,仅对同步突发存储器有效,此处未用到
//        hsdram1.Init.WaitSignalPolarity=FMC_WAIT_SIGNAL_POLARITY_LOW;//等待信号的极性,仅在突发模式访问下有用
//        hsdram1.Init.WaitSignalActive=FMC_WAIT_TIMING_BEFORE_WS;   //存储器是在等待周期之前的一个时钟周期还是等待周期期间使能NWAIT
//        hsdram1.Init.WriteOperation=FMC_WRITE_OPERATION_ENABLE;    //存储器写使能
//        hsdram1.Init.WaitSignal=FMC_WAIT_SIGNAL_DISABLE;           //等待使能位,此处未用到
//        hsdram1.Init.ExtendedMode=FMC_EXTENDED_MODE_ENABLE;        //读写使用不同的时序
//        hsdram1.Init.AsynchronousWait=FMC_ASYNCHRONOUS_WAIT_DISABLE;//是否使能同步传输模式下的等待信号,此处未用到
//        hsdram1.Init.WriteBurst=FMC_WRITE_BURST_DISABLE;           //禁止突发写
//        hsdram1.Init.ContinuousClock=FMC_CONTINUOUS_CLOCK_SYNC_ASYNC;
...
//        FMC_NORSRAM_Extended_Timing_Init(SRDAM_Handler.Extended,&FSMC_WriteTim,SDRAM_Handler.Init.NSBank,SDRAM_Handler.Init.ExtendedMode);
//	} 
   // printf("LCD ID:%x\r\n",lcddev.id); //打印LCD ID  	
...
void LCD_ShowChar(u16 x,u16 y,uint8_t num,uint8_t size,uint8_t mode)
{  							  
    uint8_t temp,t1,t;
	u16 y0=y;
...
//color:颜色 
//num:数值(0~4294967295);	 
void LCD_ShowNum(u16 x,u16 y,u32 num,uint8_t len,uint8_t size)
{         	
	uint8_t t,temp;
	uint8_t enshow=0;						   
	for(t=0;t<len;t++)
...
//[6:1]:保留
//[0]:0,非叠加显示;1,叠加显示.
void LCD_ShowxNum(u16 x,u16 y,u32 num,uint8_t len,uint8_t size,uint8_t mode)
{  
	uint8_t t,temp;
	uint8_t enshow=0;						   
	for(t=0;t<len;t++)
